...
class Tello:
    ...
    def send_command_with_return(self, command: str, timeout: int = RESPONSE_TIMEOUT, suppress=False) -> str:
    ...
    def send_command_without_return(self, command: str, suppress=False):
    ...
    def send_control_command(self, command: str, timeout: int = RESPONSE_TIMEOUT, suppress_output=False) -> bool:
        """Send control command to Tello and wait for its response.
        Internal method, you normally wouldn't call this yourself.
        """
        response = "max retries exceeded"
        for i in range(0, self.retry_count):
            response = self.send_command_with_return(command, timeout=timeout, suppress=suppress_output)
    ...
    def send_keepalive(self):
        ...
        """
        self.send_control_command("command", suppress_output=True)
